export function createLoadingScreen() {
    ...
    frequency = .075,
    currentMarker = 0,
    visibleDuration = 100000, // lets us have a shorter invisble period than visible
...
class Dot {
  ...
  _getYPos() {  
    return this.amplitude * (Math.cos(Math.PI * (this.x/ width) * this.frequency - (this.x / 10))) + height / 2;
  }
...
let createDots = () => {
  for (var i = 0; i <= startDotCount; i++) { 
    let dot = new Dot({
      ctx: ctx,
      color: !!Math.floor(Math.random() * 4) ? [0, 0, 0] : [244, 64, 51],
      startTime: window.performance.now(),
      frequency: frequency*10,
      maxAmplitude: amplitude*10,
      maxSize: Math.random() * 100000,
      maxSpeed: .1115, // Increase the maxSpeed value to make the dots move faster
